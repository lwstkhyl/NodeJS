...
就可以获取到之前设置的cookie
##### session
基于cookie实现，形式同cookie（对象--键值对），如果需要存放大量数据，就使用session
...
- `store`存储位置，这里使用`mongo_store.create({mongoUrl:"mongodb://主机名:端口号/数据库名称"})`链接数据库进行存储
...
app.use(session({
    ...
    store: mongo_store.create({
        mongoUrl: "mongodb://127.0.0.1:27017/cookie"
    }),
    cookie: {
        httpOnly: true,
        maxAge: 1000 * 60 //60s
